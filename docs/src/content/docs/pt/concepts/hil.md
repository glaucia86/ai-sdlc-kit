...
description: Por que os HITL checkpoints existem, o que protegem e como usá-los sem transformá-los em aprovações automáticas.
---

## O que é um HITL checkpoint?

Um HITL checkpoint (Human-in-the-Loop) é uma pausa deliberada no fluxo do agente onde um humano deve revisar, validar e aprovar explicitamente um artefato antes que o fluxo continue.

Os HITL checkpoints no ai-sdlc-kit não são sugestões. São paradas obrigatórias. Nenhuma etapa subsequente começa até que o humano no checkpoint tenha:
...
## Por que o HITL não é opcional

A objeção comum aos HITL checkpoints é que eles atrasam as coisas. A posição do kit é o oposto: **pular os HITL checkpoints é o que cria atrasos**.
...
O HITL checkpoint em cada portão é o mecanismo que mantém os erros _locais_.

---

## A diferença entre um HITL real e uma aprovação automática

Um HITL checkpoint só funciona se o humano realmente lê o artefato.

Sinais de que um HITL checkpoint se tornou uma aprovação automática:
...
Sinais de um HITL checkpoint real:
...
## HITL no Flow A — Discovery

| Checkpoint | Após artefato | O que verificar |
|---|---|---|
| **HITL-A1** | `idea.md` | Ideia capturada sem viés estratégico; restrições inventadas removidas |
| **HITL-A2** | `non-technical-spec.md` | Requisitos funcionais estão completos; nada crítico foi descartado |
| **HITL-A3** | `PRD.md` | Os requisitos são testáveis; as prioridades estão corretas |
| **HITL-A4** | `technical-spec.md` | A arquitetura é sólida; as restrições são realistas |
| **HITL-A5** | `epics.md` | Os epics são independentemente entregáveis; o sequenciamento está correto |

---

## HITL no Flow B — Construção

| Checkpoint | Após artefato | O que verificar |
|---|---|---|
| **HITL-B1** | `spec-epic-N.md` + `epic-N.md` | O escopo está correto; os critérios de aceitação são completos e testáveis |
| **HITL-B2** | Lista de tarefas do `/task-init` | As tarefas são granulares; nenhuma tarefa abrange mais de uma preocupação |
| **HITL-B3** | Implementação do `/task-implement` | Os critérios de aceitação da spec estão satisfeitos; sem divergência |
| **HITL-B4** | Relatório de revisão do `/task-review` | Todos os problemas sinalizados foram endereçados ou explicitamente aceitos |
| **HITL-B5** | `ops-epic-N.md` | O plano de deploy é seguro; o rollback está definido |
...
Veja também: [Cenário de Desenvolvedor Solo](/pt/scenarios/solo-dev/) para práticas HITL adaptadas.
